...
#           densities for pairwise combinations of Scenarios S1-S3 and M1-M3 when
#           priority effects alter the shedding rates of specialist and generalist
...
for(h in 1:length(chi_F_12_vals))
{
  ...
  for(i in 1:length(delta2_vals))
  {
    parameters <- c(
      ...
      phi_F_2 <-  chi_F_1_vals[h],
      phi_F_12 <- chi_F_21_vals[h],
      phi_F_21 <- chi_F_12_vals[h],
...
for(h in 1:length(m_F_12_vals))
{
  ...
  for(i in 1:length(delta2_vals))
  {
    parameters <- c(
      ...
      phi_F_2 <-  chi_F_1_vals,
      phi_F_12 <- chi_F_21_vals,
      phi_F_21 <- chi_F_12_vals,
      delta2 <- delta2_vals[i]
    )   
    out <- ode(y = initial_conditions, times = times, func = DiffEqns, parms = parameters)
